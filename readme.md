...
This is a modern microservices-based e-commerce system built with Spring Boot 3.5.6 and Java 21, implementing advanced patterns including Event-Driven Architecture, CQRS/Event Sourcing with Axon Framework, Saga Pattern, Circuit Breaker, and Redis Caching.
...
User Request → Order Service receives POST /api/orders
User Validation → Order Service checks Redis cache, then calls User Service
Inventory Check → Order Service validates stock via Inventory Service (with Circuit Breaker)
Order Creation → Order persisted in PostgreSQL with status PENDING
Event Publishing → OrderCreatedEvent published to Kafka topic
Cache Eviction → Redis cache evicted for affected data
Order Confirmation → Status updated to CONFIRMED
Payment Processing → Payment Service consumes Kafka event asynchronously
Payment Persistence → Payment transaction saved to MongoDB
Payment Event → PaymentProcessedEvent published to Kafka
Order Update → Order Service consumes payment event and updates order status
Saga-Based Order Flow (Distributed Transaction):
User Request → POST /api/orders/saga
Command Dispatch → CreateOrderCommand sent to Axon Server
Saga Initiation → OrderProcessingSaga starts
User Validation → ValidateUserCommand → UserValidatedEvent
Inventory Reservation → ReserveInventoryCommand → InventoryReservedEvent
Payment Processing → ProcessPaymentCommand → PaymentProcessedEvent
Order Completion → CompleteOrderCommand → OrderCompletedEvent
Saga End → Transaction complete
